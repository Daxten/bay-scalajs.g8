...
trait Default {
  ...
  implicit def GetResultUserGroup(implicit e0: GR[String]): GR[UserGroup] = GR { prs =>
    import prs._
    UserGroup(<<[String])
  }
  ...
  implicit def GetResultUser(implicit e0: GR[String],
                             e1: GR[OffsetDateTime],
                             e2: GR[Option[OffsetDateTime]],
                             e3: GR[Option[String]],
                             e4: GR[Option[Int]]): GR[User] = GR { prs =>
    import prs._
  ...
  class UserTable(_tableTag: Tag) extends profile.api.Table[User](_tableTag, "User") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      (Rep.Some(email), Rep.Some(password), Rep.Some(created), lastLogin, lastAction, resetPasswordToken, Rep.Some(id)).shaped.<>({ r =>
        import r._; _1.map(_ => User.tupled((_1.get, _2.get, _3.get, _4, _5, _6, _7)))
  ...
  implicit def GetResultUserToUserGroup(implicit e0: GR[Int], e1: GR[String]): GR[UserToUserGroup] = GR { prs =>
    import prs._
    val r = (<<[Int], <<[String])
    import r._
  ...
  class UserToUserGroupTable(_tableTag: Tag) extends profile.api.Table[UserToUserGroup](_tableTag, "UserToUserGroup") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      (Rep.Some(userId), Rep.Some(groupName)).shaped.<>({ r =>
        import r._; _1.map(_ => UserToUserGroup.tupled((_1.get, _2.get)))
      }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val userTableFk =
      foreignKey("UserToUserGroup_userId_fkey", userId, users)(r => r.id, onUpdate = ForeignKeyAction.NoAction, onDelete = ForeignKeyAction.NoAction)

    /** Foreign key referencing userGroups (database name UserToUserGroup_groupName_fkey) */
    lazy val userGroupTableFk = foreignKey("UserToUserGroup_groupName_fkey", groupName, userGroups)(r => r.name,
