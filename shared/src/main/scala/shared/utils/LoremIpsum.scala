...
object LoremIpsum {
  ...
  def words(count: Int): String =
    if (count > 0) (randomWord + " " + words(count - 1)).trim()
    else
      ""
  ...
  def paragraph(useStandard: Boolean = false): String =
    if (useStandard) standard else sentences(random.nextInt(3) + 2)

  def paragraph: String = paragraph(false)

  def paragraphs(count: Int, useStandard: Boolean = false): List[String] =
    if (count > 0) paragraph(useStandard) :: paragraphs(count - 1, useStandard) else Nil
}
